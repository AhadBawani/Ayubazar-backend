const Company = require('../../Models/CompanyModel');
const Products = require('../../Models/ProductsModel');
const Categories = require('../../Models/CategoryModel');
const path = require('path');

module.exports.ADD_COMPANY = async (req, res) => {
    const { companyName, gstNumber, address, city, pincode } = req.body;

    try {
        await Company.findOne({ companyName: companyName, delete: false })
            .exec()
            .then(async (companyResponse) => {
                if (companyResponse) {
                    res.status(409).send({
                        message: "Company already exists!"
                    })
                }
                else {
                    if (!req.files || Object.keys(req.files).length === 0) {
                        return res.status(400).json({ message: 'No files were uploaded.' });
                    }
                    const companyImage = req.files.companyImage;
                    const uploadPath = path.join(__dirname, '..', '..', 'Images', 'CompanyImages', companyImage.name);
                    await companyImage.mv(uploadPath);

                    const company = new Company({
                        companyName: companyName,
                        companyImage: companyImage.name,
                        address: address,
                        city: city,
                        gstNumber: gstNumber,
                        pincode: pincode
                    }).save();

                    company.then(company => {
                        res.status(201).json({
                            message: "company created successfull!",
                            company: {
                                _id: company._id,
                                companyName: company.companyName,
                                companyImage: company.companyImage,
                                address: company.address,
                                gstNumber: company.gstNumber,
                                city: company.city,
                                pincode: company.pincode
                            }
                        });
                    })
                }
            })
    }
    catch (error) {
        console.log('error in add company controller : ', error);
    }
}

module.exports.EDIT_COMPANY = async (req, res) => {
    const companyId = req.params.id;

    try {
        const companyResponse = await Company.findById(companyId);

        if (!companyResponse) {
            return res.status(404).json({ message: "Company not found!" });
        }

        const { companyName, gstNumber, address, city, pincode } = req.body;

        if (req.files) {
            const companyImage = req.files.companyImage;
            const uploadPath = path.join(__dirname, '..', '..', 'Images', 'CompanyImages', companyImage.name);
            await companyImage.mv(uploadPath);
            companyResponse.companyImage = companyImage.name
        }
        companyResponse.companyName = companyName;
        companyResponse.gstNumber = gstNumber;
        companyResponse.address = address;
        companyResponse.city = city;
        companyResponse.pincode = pincode;
        const updateResponse = companyResponse.save();

        res.status(200).json({
            message: "Company updated successfully!",
            company: updateResponse
        });
    }
    catch (error) {
        console.log('error in edit company controller : ', error);
    }
}

module.exports.GET_ALL_ONLY_COMPANIES = async (req, res) => {
    try {
        const companies = await Company.find({ delete: false }).exec();
        res.status(200).json(companies);
    }
    catch (error) {
        console.log('error in getting all the companies only controller : ', error);
    }
}

module.exports.GET_ALL_COMPANY = async (req, res) => {
    const companies = await Company.find({ delete: false }).exec();
    const companyList = [];
    try {
        for (const company of companies) {
            const products = await Products.find({ productCompany: company._id, delete: false });
            if (products.length > 0) {
                const categories = await Categories.find({ company: company._id });
                companyList.push({
                    _id: company._id,
                    companyName: company.companyName,
                    companyImage: company.companyImage,
                    products: products,
                    categories: categories
                });
            }
        }
        res.status(200).json(companyList);
    }
    catch (error) {
        console.log('error in get all company controller : ', error);
    }
}

module.exports.DELETE_COMPANY = async (req, res) => {
    const companyId = req.params.id;

    try {
        await Company.findById(companyId)
            .exec()
            .then(async (companyResponse) => {
                if (companyResponse) {
                    await Products.updateMany({ productCompany: companyId }, { delete: true }).exec();
                    await Company.findByIdAndUpdate(companyId, { delete:true }, { new:true })
                        .exec()
                        .then((deleteResponse) => {
                            if (deleteResponse) {                                
                                res.status(200).json({
                                    message: "Company deleted successfully!"
                                })
                            }
                        })
                }
                else {
                    res.status(400).json({
                        message: "Invalid response!"
                    })
                }
            })
    }
    catch (error) {
        console.log('error in delete company : ', error);
    }
}