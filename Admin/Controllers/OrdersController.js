               ...
               .then((orderResponse) => {
                    if (orderResponse) {
                         const mailOptions = {
                              ...
                              to: orderResponse.userId.email,
                              subject: 'Order Out For Delivery!',
                              html: `
                                 <p>Your order has been dispatched for Delivery.</p>                                 
                                 <p>Your order ID was <b>${orderResponse.orderId}</b>.</p>
               ...
               .then((response) => {
                    if (response) {
                         const mailOptions = {
                              ...
                              html: `
                                 <p>Your order cancellation request has been accepted.</p>                                 
                                 <p>Your order ID was <b>${response.orderId}</b>.</p>
                                 <p>Current status: <b>${response.status}</b>.</p>
                             `
                         };
                         transporter.sendMail(mailOptions, (error, info) => {
                              if (error) {
               ...
               .then((response) => {
                    if (response) {
                         const mailOptions = {
                              from: 'admin@ayubazar.com',
                              to: response.userId.email,
                              subject: 'Order Cancelled Request Reject!',
                              html: `
                                 <p>Your order cancellation request has been rejected.</p>
                                 <p>Your order ID was <b>${response.orderId}</b>.</p>
                                 <p>Current status: <b>${response.status}</b>.</p>
                             `
                         };
                              ...
                              .then((response) => {
                                   if (response) {
                                        const mailOptions = {
                                             ...
                                             html: `
                                                <p>Your order has been delivered to your shipping address.</p>                                 
                                                <p>Your order ID was <b>${response.orderId}</b>.</p>
                                                <p>Current status: <b>${response.status}</b>.</p>
                                             `
                                        };
