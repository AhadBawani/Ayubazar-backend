...
module.exports.ADD_PRODUCT = async (req, res) => {
    const { productName, productCompany, description, taxStatus, taxClass, discount,
        productSubCategory, options, bulletDescription, productCategory } = req.body;
    try {
        ...
        res.status(201).json({
            ...
            product: {
                ...
                productSubCategory: savedProduct.productSubCategory,
                taxClass: savedProduct.taxClass,
                taxStatus: savedProduct.taxStatus,
...
module.exports.EDIT_PRODUCT = async (req, res) => {
    ...
    try {
        ...
        const { productName, productCompany, description, category, subCategory, discount,
        ...
        if (discount > 0) {
            const updatedOptions = JSON.parse(productResponse.originalOptions)?.map((opt, index) => {
                const price = parseFloat(opt.price);
                const discountedPrice = Math.round(price - (price * discount) / 100);
                return { ...opt, price: discountedPrice.toString() };
            })
            await Product.findByIdAndUpdate(productResponse._id,
                {
                    options: JSON.stringify(updatedOptions),
                },
                { new: true });
