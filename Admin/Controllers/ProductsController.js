...
module.exports.ADD_PRODUCT = async (req, res) => {
    const { productName, productCompany, description, taxStatus, taxClass, discount, isVariationAvailable,
        productSubCategory, options, bulletDescription, productCategory, salesPrice, codAvailable } = req.body;
    try {
        const existingProduct = await Product.findOne({ productName, productCompany, delete: false }).exec();
        if (existingProduct) {
        ...
        const newProduct = new Product({
            ...
            taxClass: taxClass,
            taxStatus: taxStatus,
            salesPrice: parseInt(salesPrice) > 0 ? parseInt(salesPrice) : null,
            originalSalesPrice: parseInt(salesPrice) > 0 ? parseInt(salesPrice) : null,
            productSubCategory: productSubCategory,
            outOfStock: false,
            discount: parseInt(discount),
            delete: false,
            isVariationAvailable: isVariationAvailable,
            codAvailable: codAvailable
        });
        const savedProduct = await newProduct.save();

        if (discount > 0) {
            if (!savedProduct.isVariationAvailable && savedProduct.salesPrice > 0) {
                let discountAmount = Math.floor((savedProduct.originalSalesPrice * discount) / 100);
                let diffPrice = savedProduct.originalSalesPrice - discountAmount;
                ...
                const updatedOptions = JSON.parse(newProduct.originalOptions)?.map((opt, index) => {
...
module.exports.ADD_PRODUCT_THROUGH_EXCEL = async (req, res) => {
    try {
        ...
        worksheet.eachRow((row, rowNumber) => {
            ...
            const isVariationAvailable = row.getCell(2).value === 'simple' ? false : true;
            const product = {
            ...
            if (!product.productCategory) {
                product.productCategory = 'Narayani Product';
            }
            if (!product.productSubCategory) {
                product.productSubCategory = 'Narayani Product';
            }
        ...
        for (let i = 0; i < products.length; i++) {
            await Companies.findOne({ companyName: products[i].productCompany })
                ...
                .then(async (companyResponse) => {
                    if (companyResponse) {
                        await Categories.findOne({ category: products[i].productCategory })
                            .exec()
                            .then(async (categoryResponse) => {
                                if (categoryResponse) {
                                    let subCategoryResponse = await SubCategory.findOne({
                                        category: categoryResponse._id,
                                        subCategory: products[i].productSubCategory
                                    }).exec();

                                    // If the subcategory doesn't exist, create it
                                    if (!subCategoryResponse) {
                                        subCategoryResponse = await new SubCategory({
                                            category: categoryResponse._id,
                                            subCategory: products[i].productSubCategory
                                        }).save();
                                    }
                                    await new Product({
                                        ...
                                        productSubCategory: subCategoryResponse._id,
                                        description: products[i]?.simpleDescription?.length > 0 ? JSON.stringify(products[i]?.simpleDescription) : [],
                                        bulletDescription: products[i]?.bulletDescription?.length > 0 ? JSON.stringify(products[i]?.bulletDescription) : [],
                                        ...
                                        isVariationAvailable: products[i]?.isVariationAvailable,
                                        originalOptions: JSON.stringify(products[i]?.productOptions),
                                        originalSalesPrice: products[i]?.SalesPrice,
                                        delete: false,
                                    ...
                                    const subCategoryResponse = await new SubCategory({
    ...
    try {
        await Product.findByIdAndUpdate(productId, { outOfStock: true }, { new: true })
            ...
            .then(async (updateResponse) => {
                if (updateResponse) {
                    await UserCart.deleteMany({ productId: productId }).exec();
                    res.status(200).send({
...
module.exports.EDIT_PRODUCT = async (req, res) => {
    ...
    try {
        ...
        productResponse.description = !description ? [] : description;
        productResponse.options = options;
        productResponse.originalOptions = options;
        productResponse.discount = discount;
        productResponse.salesPrice = salesPrice;
        productResponse.originalSalesPrice = salesPrice;
        productResponse.taxClass = taxClass;
        productResponse.taxStatus = taxStatus;
        productResponse.bulletDescription = !bulletDescription ? [] : bulletDescription;
        productResponse.productCategory = category;
        productResponse.productSubCategory = subCategory;
        productResponse.isVariationAvailable = isVariationAvailable;
        productResponse.codAvailable = codAvailable;

        const updatedResponse = await productResponse.save();
        // console.log('updated Response : ', updatedResponse);
        if (parseInt(discount) > 0) {
            if (isVariationAvailable == 'true') {
                const updatedOptions = JSON.parse(updatedResponse.originalOptions)?.map((opt, index) => {
                    const price = parseFloat(opt.price);
                    const discountedPrice = Math.round(price - (price * discount) / 100);
                    return { ...opt, price: discountedPrice.toString() };
                })
                await Product.findByIdAndUpdate(updatedResponse._id,
                    {
                        options: JSON.stringify(updatedOptions),
                    },
                    { new: true });
            } else {
                let discountAmount = Math.round((updatedResponse.originalSalesPrice * discount) / 100);
                let diffPrice = updatedResponse.originalSalesPrice - discountAmount;
                await Product.findByIdAndUpdate(updatedResponse._id,
                    {
                        salesPrice: diffPrice,
                    },
...
module.exports.GET_BEST_SALING_PRODUCTS = async (req, res) => {
    try {
        await Product.find({ bestSelling: true, delete: false })
            .exec()
            .then((productResponse) => {
                res.status(200).json(productResponse);
            })
    ...
    try {
        await Companies.findOne({ companyName: 'Narayani' })
            ...
            .then((companyResponse) => {
                if (companyResponse) {
                    Product.find({ productCompany: companyResponse._id, delete: false }).exec()
                        .then((productResponse) => {
                            if (productResponse) {
                                res.status(200).json(productResponse);
                            }
...
module.exports.DELETE_PRODUCT = async (req, res) => {
    ...
    try {
        await Product.findById(productId)
            ...
            .then(async (productResponse) => {
                if (productResponse) {
                    await Product.findByIdAndUpdate(productId, { delete: true }, { new: true })
                        .exec()
                        .then(async (updatedProductResponse) => {
